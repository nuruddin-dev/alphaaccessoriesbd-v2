...
exports.caculateTaxAmount = order => {
  try {
    ...
    if (order.products && order.products.length > 0) {
      order.products.map(item => {
        ...
        if (item.status !== 'Cancelled') {
          if (item.product?.popular && item.priceWithTax === 0) {
    ...
    const deliveryCharge = getState().user.isInDhakaCity ? 50 : 100;
    order.totalWithTax = order.total + order.totalTax + deliveryCharge;
