...
// @route PUT api/customer/:id/add-invoice
// @desc Add an invoice to a customer's purchase history
// @access Private
router.put(
  '/:id/add-invoice',
  auth,
  ...
  async (req, res) => {
    try {
      ...
      if (!invoiceId) {
        return res
          .status(400)
          .json({ error: 'You must provide an invoice ID.' });
      }
      ...
      // Add invoice to purchase history if not already there
      if (!customer.purchase_history.includes(invoiceId)) {
        customer.purchase_history.push(invoiceId);
      }

      // Update due amount if provided
      if (amount) {
        customer.due += Number(amount);
      }
      ...
      res.status(200).json({
        success: true,
        message: 'Invoice added to customer history successfully!',
        customer: updatedCustomer
      });
    } catch (error) {
      res.status(400).json({
...
// @route PUT api/customer/:id/update-due
// @desc Update customer's due amount
// @access Private
router.put(
  '/:id/update-due',
  auth,
  ...
  async (req, res) => {
    try {
      const customerId = req.params.id;
      const { amount, isPayment } = req.body;

      if (amount === undefined) {
        return res.status(400).json({ error: 'You must provide an amount.' });
      }
      ...
      // If isPayment is true, reduce the due amount, otherwise increase it
      if (isPayment) {
        customer.due -= Number(amount);
      } else {
        customer.due += Number(amount);
