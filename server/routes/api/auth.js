  ...
  try {
    ...
    if (!email) {
      return res.status(400).json({
        error: 'আপনাকে অবশ্যই মোবাইল নম্বর অথবা মেইল এড্রেস দিতে হবে'
      });
    }
    ...
    if (!password) {
      return res
        .status(400)
        .json({ error: 'আপনাকে অবশ্যই পাসওয়ার্ড দিতে হবে' });
    }

    const user = await User.findOne({ email });
    if (!user) {
      return res.status(400).send({ error: 'কোন একাউন্ট নেই' });
    }

    if (user && user.provider !== EMAIL_PROVIDER.Email) {
      return res.status(400).send({
        error: `এই মেইল এড্রেস দিয়ে ${user.provider} দ্বারা একাউন্ট বিদ্যমান`
      });
    }
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!firstName) {
      return res.status(400).json({ error: 'আপনাকে অবশ্যই নাম দিতে হবে' });
    }
    ...
    if (existingUser) {
      return res.status(400).json({ error: 'একাউন্ট বিদ্যমান, লগইন করুন' });
    }
